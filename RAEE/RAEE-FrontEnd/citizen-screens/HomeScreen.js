  ...
  const loadLocations = async () => {
    try {
      ...
      if (response && response.success && response.data) {
        // Filtrar solo ubicaciones con estado = 1 (activas)
        // Nota: Estado_Ubicaciones viene como string "1" desde la base de datos
        const activeLocations = response.data.filter(location => location.Estado_Ubicaciones === "1" || location.Estado_Ubicaciones === 1);

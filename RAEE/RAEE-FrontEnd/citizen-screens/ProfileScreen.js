...
export default function ProfileScreen({ navigation }) {
  ...
  const loadUserPoints = async () => {
    try {
      ...
      const pointsData = await ApiService.getUserPoints(user.idUsuarios);
      if (pointsData && pointsData.success) {
        setUserPoints(pointsData.data.puntos || 0);
      }
    } catch (error) {
  ...
  const loadUserPublicationsCount = async () => {
    try {
      ...
      const publicationsData = await ApiService.getUserPublications(user.idUsuarios, 1, 1);
      
      if (publicationsData && publicationsData.success) {
        // Obtener el total de publicaciones del usuario desde la paginación
        const totalCount = publicationsData.pagination?.total || 0;
        setUserPublicationsCount(totalCount);
      } else {
        setUserPublicationsCount(0);
      }
    } catch (error) {
  ...
  const selectImage = async (image) => {
    ...
    try {
      ...
      const updateResponse = await ApiService.updateProfile({
        ImagenPerfil_Usuarios: image.name
      });
      ...
      if (updateResponse.success) {
        ...
        console.error('[ERROR] No se pudo guardar la imagen en la base de datos:', updateResponse.message);
        Alert.alert('Error', 'No se pudo guardar la imagen en la base de datos');
        return;
      }

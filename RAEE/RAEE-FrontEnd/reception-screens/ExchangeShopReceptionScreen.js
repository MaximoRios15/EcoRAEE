...
export default function ExchangeShopScreen({ navigation }) {
  // Datos simulados para reemplazar useAuth
  const user = { 
    nombres: 'Usuario', 
    apellidos: 'Recepción',
    imagenPerfil: null,
    Puntos_Usuarios: 1500
  };
  ...
  const [userPoints, setUserPoints] = useState(user?.Puntos_Usuarios || 1500);
  ...
  useEffect(() => {
    // Comentado: loadEquipment();
    // Comentado: loadCategories();
    // Comentado: loadStates();
    loadThemePreference();
...
const renderSidebarItem = (title, iconName, color, onPress) => (
  ...
  const loadCategories = async () => {
    ...
    console.log('Categorías cargadas (simuladas)');
  };
  ...
  const loadStates = async () => {
    ...
    const mockStates = [
      { id: 1, name: 'Funcional' },
      { id: 2, name: 'Parcialmente funcional' },
      { id: 3, name: 'No funcional' },
      { id: 4, name: 'Para repuesto' }
    ];
    setStates(mockStates);
    console.log('Estados cargados (simulados)');
  };
  ...
  const loadEquipment = async () => {
    ...
    const mockEquipment = [
      {
        ...
        brand: 'Apple',
        pointsPrice: 850,
        image: getImageByCategory(4),
        category: 'teléfonos',
        categoryId: 4,
        stateId: 1,
        shipping: 'Calle 123, Bogotá',
        installments: '',
        description: 'iPhone en excelente estado',
        weight: 0.2,
        dimensions: '14.7 x 7.2 x 0.7 cm',
      ...
      },
      {
        ...
        installments: '',
        description: 'Smart TV en perfecto estado',
        weight: 15.5,
        dimensions: '123 x 71 x 8 cm',
        accessories: 'Control remoto, cables',
        state: 'Funcional',
    ...
    setIsLoading(false);
    console.log('Equipos cargados (simulados)');
  };
